...
This document describes the technical specifications of the MSE process. It is a living document that will be continually updated to reflect the current state of the MSE work. Comments, questions, and feedback are welcome by contacting the [MSE Technical Group Members](https://safmc-mse.netlify.app/). 

More information on the MSE process can be found on the [SAFMC Snapper-Grouper MSE homepage](https://safmc-mse.netlify.app/).


## The MSE Framework

The MSE is being conducted with the `openMSE` framework. `openMSE` is an R package that has been developed for conducting fast, flexible, and transparent, MSE for a wide range of fisheries. `openMSE` is an umbrella package that includes the `MSEtool`, `SAMtool` and `DLMtool` packages. A non-technical description of `openMSE` and its key features is available on the [`openMSE` website](https://openmse.com/). 
...
All code related to the MSE process is available in the [SAFMC-MSE Github Repository](https://github.com/Blue-Matter/SAFMC-MSE). The repository is currently set to private and only accessible to MSE Technical Group. Please contact the [MSE Technical Group](https://safmc-mse.netlify.app/) if you would like access. 
...
The MSE process is currently focused on two main species: Red Snapper (*Lutjanus campechanus*) and Gag Grouper (*Mycteroperca microlepis*). The most recent stock assessments for these two stocks were conducted in 2021 ([SEDAR 73](https://sedarweb.org/assessments/sedar-73/) and [SEDAR 71](https://sedarweb.org/assessments/sedar-71/) for Red Snapper and Gag respectively).
...
### Calculating Seasonal Fleet Structure
...
The Discard fleets include both discarded catch from the closed seasons and discards that occur during the fishing season. To be able to evaluate management actions such as different season lengths, or minimum size limits, the operating model requires a different fleet structure to that used in the assessment. 

The fleet structure for the operating model requires the removals to be split into On-Season (landings and discards that occur during the mini-seasons when the fishery is open) and Off-Season (discards of all catch when the fishery is closed). This results in an OM with 6 fleets:
...
The SEDAR 73 model was converted to the OM with the On- and Off-Season structure with the following steps:

1. Combine Landings and Discards from each fleet into Total Removals
2. Calculate proportion of Total Removals taken during On- and Off-Season for each fleet and year
3. Allocate proportion of total fishing mortality for each fleet into On- and Off-Seasons based on the proportion of removals in each season.

This process requires information on the proportion of total removals that occur during the On- and Off-Seasons, as well as information on the selectivity and retention patterns of the fleets during these seasons. 

The steps are described in more detail below.

#### Combine Landings and Discards into Total Removals

Total removals for each fleet (Commercial Handline, Recreational Headboat, and General Recreational) were calculated by summing the landings and discards predicted by the SEDAR 73 model. This results in a model that describes the total removals-at-age by year for the three fleets.
...
>> Note: assumes Recreational Headboat fleet follows recreational fishing season 
...
>> Note: currently assumes General Recreational Fleet behavior is the same as Recreational Headboat. Need further analysis of recreational catch data (MRIP) to determine fraction of total removals taken during On- and Off-Seasons  
...
knitr::include_graphics("../img/RS_relativeF.png")
```
...
knitr::include_graphics("../img/RS_histLandings_Discards.png")
```


## Gag Grouper OM 

### OM Seasonal Fleet Structure
...
Similar to the Red Snapper model described above, the Discard fleets include both discarded catch from the closed seasons, and discards that occur during the fishing season. To be able to evaluate management actions such as different season lengths, or minimum size limits, the operating model requires a different fleet structure to that used in the assessment. 

The fleet structure for the operating model requires the removals to be split into On-Season (landings and discards that occur during the mini-seasons when the fishery is open) and Off-Season (discards of all catch when the fishery is closed). This results in an OM with 7 fleets:
...
knitr::include_graphics("../img/GG_relativeF.png")
```
...
knitr::include_graphics("../img/GG_histLandings_Discards.png")
```

## Generating Multi-Species OM
...
```{r RS_refpoints}
RS_Ref_df <- readRDS('../Objects/RS_Ref_df.rda')
```
...
knitr::include_graphics("../img/RS_Ref_Points.png")
```
...
```{r GG_refpoints}
GG_Ref_df <- readRDS('../Objects/GG_Ref_df.rda')
```
...
knitr::include_graphics("../img/GG_Ref_Points.png")
```
...
knitr::include_graphics("../img/RS_Select_hist.png")
```
...
knitr::include_graphics("../img/GG_Select_hist.png")
```
...
knitr::include_graphics("../img/RS_GG_Removals_hist.png")
```
...
knitr::include_graphics("../img/RS_GG_Landings_hist.png")
```
...
knitr::include_graphics("../img/RS_GG_Discards_hist.png")
```
...
knitr::include_graphics("../img/RS_SSB_ref_hist.png")
```
...
knitr::include_graphics("../img/GG_SSB_ref_hist.png")
```
...
```{r recdevs}
RS_rec_devs <- readRDS('../Objects/RS_rec_devs.rda')
GG_rec_devs <- readRDS('../Objects/GG_rec_devs.rda')

```
The recruitment deviations for the projection period are generated from a log-normal distribution, with the standard deviation and lag-1 auto-correlation factor reported by the stock assessments. 

For the Base Case OM, the standard deviation of the log recruitment deviations for Red Snapper was `r round(RS_rec_devs$Perr,2)` with an auto-correlation factor of `r round(RS_rec_devs$AC,2)`. Figure \@ref(fig:RS-rec-devs) shows an example of the recruitment deviations for 9 simulations for Red Snapper. 
...
knitr::include_graphics("../img/RS_rec_devs.png")
```

For the Base Case OM, the standard deviation of the log recruitment deviations for Gag Grouper was `r round(GG_rec_devs$Perr,2)` with an auto-correlation factor of `r round(GG_rec_devs$AC,2)`.  Figure \@ref(fig:GG-rec-devs) shows an example of the recruitment deviations for 9 simulations for Gag Grouper. 
...
knitr::include_graphics("../img/GG_rec_devs.png")
```
...
### Set an Annual Catch Limit (ACL)


For a given ACL for Red Snapper, 28.07% is allocated to the Commercial fleet and 71.93% to the Recreational sector (SEDAR 73). The ACL is assumed to apply to the On-Season (directed) fleets. 

For Gag Grouper, 51% of the total ACL is allocated to the commercial fishery, and the remaining 49% to the recreational sector (SEDAR 71). 

For both stocks, the ACL is assumed to apply to the On-Season (directed) fleets.
